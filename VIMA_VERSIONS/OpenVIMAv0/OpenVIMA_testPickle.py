...
This is Intelectual Property of Nicholas Pacheck
---------------------------------------------------
Engineer: Pacheck, Nicholas
Date: 01/25/23
---------------------------------------------------
Description:
This is a gui module for the OpenVima Robot control REPO. The implemented
GUI provides the user with several modes of control.

    1) Forward Kinematics ( through sliders )
    2) Inverse Kinematics ( through sliders )
    3) FK MIMIC ( Copies limb angles to acutators )
    4) IK MIMIC ( IK computed on my finger location in (XYZ) space
    5) Trainer: Able to train and string together movments to complete task


TODO:
    ADD TRAINING MODULE
    INTERFACE HELL
    Debuf interface within gui
    Display arduino message in GUI
...
class RobotControlGUI:
    ...
    def main_loop(self):
        ...
        try:
            ...
            for i, i_serial in enumerate(self.serialList):
                try:
                    if self.shaken_list[i]:
                        ...
                        if self.verbose:print("not shaken", zip(self.shaken_list, self.device_names))
                except IndexError as IE:
                    if self.verbose:print("INDEX ERROR DURING SENDING", IE)
            ...
            #Calculate loop time
            end_time = time.time()
            if self.verbose:print("TIME:", self.start_time - end_time)
    ...
    def activate_ik(self):
        if self.verbose:print("INVERSE KINEMATICS ACTIVATED")
    ...
    def activate_hand_tracking(self):
        ...
        # Get the coordinates defined by the inverse kinematic sliders on right
        slider_coord = self.get_slider_list(self.ik_sliders)
        # reorder the list of the sliders to satisfy the order of the IKin solver
        #TODO: Define a function to collect hand orentationa and use that within the ik sovler
    ...
    def activate_fk(self):
        if self.verbose:print("FORWARD KINEMATICS ACTIVATED")
    ...
    def activate_trainer(self):
        ...
        if self.verbose:print(f"Trainer has been called, writing prestored movments:{requestList}")
    ...
    def start_training(self, requestList):
        if self.verbose:print("GATHERING TRAINING DATA")
    ...
    def update_connection_status(self):
        for i, label in enumerate(self.device_label_list):
            try:
                if self.shaken_list[i]:
                    if self.verbose:print(f"Serial Connection made with {self.device_names[i]}")
                    label.configure(fg_color="darkgreen")
            except IndexError as IE:
                if self.verbose:print(IE, "issue with index raised")
