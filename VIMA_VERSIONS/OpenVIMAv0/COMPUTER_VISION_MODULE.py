...
Description:
    ...
    This module does all the computer vision processing for both the forward
    and inverse kinematic features of OpenVIMA. Implying ComputerVisionBody class
    is an implementation of sensorfusion, fuseing the CV hand results with the
    CV Body results for a holistic view.
...
class ComputerVisionBody:
    ...
    def compute_cv_angle(self, p1, p2, p3, v2 = None):
        '''THIS FUNCTION COMPUTES THE ANGLE BETWEEN 3 POINTS P1, P2, AND P3
            ...
            MODIFICATIONS:
                ...
                The modifications made are to insted
                calculitng the angle only in the xy-plane, we will
                give option to define a vector for coordiant 3 such that
                you can compute the angle component in
                and plane oreientation.
        ...
        try:
            ...
            # the modifciation ...
            # if not defualt operation, hence using the added operation descirbed in modifcations
            if v2 is not None:
                # unpack coordiantes from inputed list
                vx, vy, vz = v2
            else:
                # default usage of the method
    ...
    def find_base_angle(self):
        ...
        try:
            ...
            # Define the center coordinate of the user
            center_coords = [

                # Do so by computing average of each coordinate being inputed into the funciton
                (self.cx_list[23] + self.cx_list[24])/2,
                (self.cy_list[23] + self.cy_list[24])/2,
                (self.cz_list[23] + self.cz_list[24])/2

            ]

            # Define a vector from the center coordinte to the horizontla hip s.t user faces camera
            horiz_vector = [self.cx_list[self.rot_hip], self.cy_list[self.rot_hip], 0]
            # Define a vector from the center coordinate to the current gip position
            hip_vector = [self.cx_list[self.rot_hip], self.cy_list[self.rot_hip], self.cz_list]
            # Compute the dot product of the vectors
            aDotb = np.dot(hip_vector, horiz_vector)

            # Compute the magnitudes of each vecotr
            mag_horiz = np.sqrt(horiz_vector[0]**2 + horiz_vector[1]**2 + horiz_vector[2]**2)
            mag_hip = np.sqrt(hip_vector[0]**2 + hip_vector[1]**2 + hip_vector[2]**2)

            # Compute the angle seen froma above between the vectors

            # this is done through dot product relation
            #  A dot B = mag(A) * mag(b) * cos( theta )
            # such that   theta = arccos( (A dot B) / (mag(A)* mag(B) )
            theta = np.rad2deg(np.arccos(aDotb/(mag_hip*mag_horiz)))
    ...
    def get_3dof_angles(self):
        try:
            base = float(self.find_base_angle())
...
def inLoop_vision_body(vision):
    ...
    print(f"thetaList: {newThetaList}")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        newThetaList, personBool, img, shoulderBool, suc, ik_coords = inLoop_vision_body(visionc)
