...
class Arduino:
    ...
    # Sends a message string and status bits arduino with the lat message mbit being hte staus bit
    ...
    def dual_handshake(self):
        # if standard connection is made with arduino
        if self.connected:
            # shake hands with break word SHAKING
            self.shaken = self.handshake("SHAKING")
            # once break from the SHAKING handshake
            if self.shaken:
                # arduino will continually send READY after executing each instruction
    ...
    def handshake(self, myBreakWord):
        ...
        try:
            ...
            # the word the computer is looking for to recive to break from handshake buffer
    ...
    def send_instructionSet(self, instructionsSet, reset):
        ...
        # for each instruction wanting to be sent, send instruciton in order
        # if reset is high, then the new intruction repalces the old intstruction
        # if reser is low, then the new instruction gets appended into the end of buffer
    ...
    # Method for sending more than one instruction at a time, for instance if you want to send 5 in order
